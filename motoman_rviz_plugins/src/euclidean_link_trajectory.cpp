...
namespace motoman_rviz_plugins {
  EuclideanLinkTrajectory::EuclideanLinkTrajectory() {
    link_name_property_ = new rviz::StringProperty("link name", "", "", this,SLOT(updateLinkName()));
    alpha_property_ = new rviz::FloatProperty("alpha", 1.0, "alpha channel value", this, SLOT(updateAlpha()));
    color_property_ = new rviz::ColorProperty("color", QColor(25, 255, 240), "color of trajectory", this, SLOT(updateColor()));
    line_width_property_ = new rviz::FloatProperty("line_width", 0.01, "line width", this, SLOT(updateLineWidth()));
  ...
  void EuclideanLinkTrajectory::processMessage(const motoman_viz_msgs::EuclideanLinkTrajectory::ConstPtr &msg) {

    ROS_DEBUG("!! Rviz Plugin Call Back !!");
    ...
    line_->setLineWidth(line_width_);
    line_->setColor(color_.red() * 255.0, color_.green() * 255.0,
                    color_.blue() * 255.0, alpha_);
